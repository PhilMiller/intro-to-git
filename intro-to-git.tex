...
\author{Phil Miller}%, \href{mailto:phil@intensecomputing.com}{phil@intensecomputing.com}
%Based on original slides by Bart Trojanowski}
\institute{Intense Computing, for Modern Electron}
\date{June 23, 2021}
...
#3{cherry}           & #4{http-fetch}        & #4{prune}          & #2{switch}             \\
#2{cherry-pick}      & #4{http-push}         & #4{prune-packed}   & #4{symbolic-ref}       \\
...
#4{cvsimport}        & #4{ls-remote}         & #3{remote}         & #4{upload-pack}        \\
...
#4{diff-files}       & #4{merge-base}        & #2{reset/restore}  & #4{write-tree}         \\
...
\item Who
  \begin{itemize}
    \item authored a particular revision?
    \item incorporated that revision?
  \end{itemize}
\item When?
\pause{}
\item Why?
...
\begin{frame}
\frametitle{What this buys us}
\begin{itemize}
\item What did I just change? (i.e. Why is it crashing?! ARRRGGGHHH)
\item What are people working on?
\item Who knows about feature/component X?
\item How did bug Y get fixed? Which version has that fix?
\item What code was used for these experiments?
\end{itemize}

\end{frame}

% =====================================================================
\mysubsection{Version Control components}{concepts:components}
% ---------------------------------------------------------------------
\begin{frame}
\frametitle{Version Control components}
\begin{columns}[t]
...
\begin{columns}[t]
        ...
        \begin{column}[T]{.5\textwidth}
                \vspace{.2\textheight}
                \includegraphics[width=0.5\linewidth]{repo-blob.eps}
        \end{column}
\end{columns}
...
% =====================================================================
\mysubsection{Version Control operations}{concepts:operations}

% ---------------------------------------------------------------------
\begin{frame}
\frametitle{Version Control operations}
Bootstrap
\begin{itemize}
        \item init
        \item clone
...
% ---------------------------------------------------------------------
\begin{frame}
\frametitle{Version Control operations: Communication}
...
\includegraphics[width=.5\linewidth]{struct-00.eps}
\vspace{\baselineskip}
\begin{center}
        the history
\end{center}
...
\vspace{0.5\baselineskip}
\begin{itemize}
\item  \CMD{restore} \quad (preferred, since v2.23)
\item        \CMD{checkout}
\item        \CMD{read-tree}
\item        \CMD{reset}
\end{itemize}
...
\CMD{.git} \\
\begin{itemize}
  \item Repository data directory
\end{itemize}
...
\begin{columns}[t]
        ...
        \end{column}
        \begin{column}[T]{.5\textwidth}
                \includegraphics[width=.8\linewidth]{obj-05-imutable-2.eps}
        \end{column}
\end{columns}
\end{frame}
...
\begin{columns}[t]
        ...
        \end{column}
        \begin{column}[T]{.5\textwidth}
                \includegraphics[width=.8\linewidth]{obj-05-imutable-3.eps}
        \end{column}
\end{columns}
\end{frame}
...
\CMD{bart@jukie.net/~git/project.git/}
        ...
        \item ssh protocol to large hosting sites
                \begin{Verbatim}[commandchars=\\\{\}]
\CMD{ssh://\red{git}@github.com/ModernElectron/project.git/}
\CMD{\red{git}@github.com/ModernElectron/project.git/}
                \end{Verbatim}
\end{itemize}
\end{frame}
...
\CMD{\$ git switch -c name \fnt{commit}}
\begin{itemize}
        \item checkout files from ``name'' branch
\end{itemize}
        ...
        \begin{column}[T]{.5\textwidth}
                ...
                \vspace{\baselineskip}
                       {\small
                         (since v2.23)
                \begin{Verbatim}[commandchars=\\\{\}]
\CMD{\$ git switch -c wicked master}
                \end{Verbatim}
                }
...
\CMD{\$ git checkout -b wicked master}
                ...
                \vspace{\baselineskip}
                       {\small
                         (equivalently)
                \begin{Verbatim}[commandchars=\\\{\}]
\CMD{\$ git branch wicked master}
\CMD{\$ git switch wicked} or \CMD{\$ git checkout wicked}
                \end{Verbatim}
                }
...
% ---------------------------------------------------------------------
\begin{frame}
\frametitle{Cherry-picking (e.g. to share upstream)}

\begin{columns}[t]
  \begin{column}[T]{0.6\textwidth}
    D is private, but E should be contributed
  \end{column}
  \begin{column}[T]{.4\textwidth}
    \includegraphics[width=\linewidth]{cherry-pick-01.eps}
  \end{column}
\end{columns}
\end{frame}
...
\begin{columns}[t]
  \begin{column}[T]{0.6\textwidth}
    D is private, but E should be contributed \\
    {\small
    \CMD{\$ git switch -c share-upstream}
    }
  \end{column}
  \begin{column}[T]{.4\textwidth}
    \includegraphics[width=\linewidth]{cherry-pick-02.eps}
  \end{column}
\end{columns}
\end{frame}
...
\begin{columns}[t]
  ...
  \end{column}
  \begin{column}[T]{.4\textwidth}
    \includegraphics[width=\linewidth]{cherry-pick-03.eps}
  \end{column}
\end{columns}
\end{frame}
...
\includegraphics[width=.5\linewidth]{remote-00.eps}
...
\includegraphics[width=.5\linewidth]{remote-01.eps}
...
{\tiny
\begin{Verbatim}[commandchars=\\\{\}]
\CMD{\$ git clone ssh+git://repo.or.cz/project.git} \red{project}
\black{Initialize \red{project}/.git}
\black{Initialized empty Git repository in /tmp/\red{project}/.git/}
\faint{remote: Counting objects: 77575, done.}
\faint{remote: Compressing objects: 100% (26407/26407), done.}
...
\includegraphics[width=.5\linewidth]{remote-02.eps}
...
\includegraphics[width=.5\linewidth]{remote-03.eps}
...
\includegraphics[width=.5\linewidth]{remote-04.eps}
...
\includegraphics[width=.5\linewidth]{remote-05.eps}
...
\includegraphics[width=.5\linewidth]{remote-06.eps}
...
\includegraphics[width=.5\linewidth]{remote-07.eps}
...
\includegraphics[width=.5\linewidth]{remote-08.eps}
...
\faint{From repo.or.cz:project.git}
...
\includegraphics[width=.5\linewidth]{remote-09.eps}
...
\faint{Receiving objects: 100% (190/190), 69.53 KiB, done.}
\faint{Resolving deltas: 100% (170/170), completed with 40 local objects.}
\red{From repo.or.cz/project.git}
...
\includegraphics[width=.5\linewidth]{remote-10.eps}
...
\includegraphics[width=.5\linewidth]{remote-11.eps}
...
\includegraphics[width=.5\linewidth]{remote-12.eps}
...
\includegraphics[width=.5\linewidth]{remote-13.eps}
...
\includegraphics[width=.5\linewidth]{remote-14.eps}
...
\includegraphics[width=.5\linewidth]{remote-15.eps}
...
\includegraphics[width=.5\linewidth]{remote-16.eps}
...
\includegraphics[width=.5\linewidth]{remote-17.eps}
...
\includegraphics[width=.5\linewidth]{remote-18.eps}
...
\includegraphics[width=.5\linewidth]{remote-19.eps}
...
\includegraphics[width=.5\linewidth]{remote-20.eps}
...
\includegraphics[width=.5\linewidth]{remote-21.eps}
...
\includegraphics[width=.5\linewidth]{remote-22.eps}
...
\includegraphics[width=.5\linewidth]{remote-23.eps}
...
\begin{center}
Fred can't push directly to \verb|master|
\end{center}
...
\includegraphics[width=.5\linewidth]{remote-24.eps}
...
\includegraphics[width=.5\linewidth]{remote-25.eps}
...
\includegraphics[width=.5\linewidth]{remote-26.eps}
...
\includegraphics[width=.5\linewidth]{remote-27.eps}
...
\includegraphics[width=.5\linewidth]{remote-28.eps}
...
\includegraphics[width=.5\linewidth]{remote-29.eps}
...
\begin{Verbatim}[commandchars=\\\{\}]
\CMD{\$ git switch -c fred-fix \red{fred/master}}
\end{Verbatim}
...
\includegraphics[width=.5\linewidth]{remote-30.eps}
...
\includegraphics[width=.5\linewidth]{remote-31.eps}
...
\includegraphics[width=.5\linewidth]{remote-32.eps}

\begin{Verbatim}[commandchars=\\\{\}]
\fnt{\$ git switch master}
...
\includegraphics[width=.5\linewidth]{remote-33.eps}
...
\includegraphics[width=.5\linewidth]{remote-34.eps}
...
\includegraphics[width=.5\linewidth]{remote-35.eps}
...
\includegraphics[width=.5\linewidth]{remote-36.eps}
...
\includegraphics[width=.5\linewidth]{remote-37.eps}
...
% =====================================================================
\mysubsection{Pull Requests}{using:pull-requests}

\begin{frame}
\frametitle{Pull Requests}

\begin{itemize}
\item Create a descriptively named branch for your work
\item Push branch to shared repo or public fork
\item Open a (draft) \emph{pull request} in destination repo \\
\quad Include ``Fixes \#12345'' in description as appropriate
  \pause{}
\item Get automated test results
  \pause{}
\item Receive and address review comments
  \pause{}
\item Branch merged
\end{itemize}
...
% =====================================================================
\mysubsection{Stash}{using:loose}
...
\begin{itemize}
        ...
        \item do that other thing
                \pause{}
                \vspace{\baselineskip}
                \item \CMD{git stash list/show}
                \pause{}
                \vspace{\baselineskip}
              \item \CMD{git stash pop/apply/drop}
\end{itemize}
\end{frame}
